...
cleanse = lambda text: re.sub(r'[\W\d_]', '', text)
...
def sort_by_commonality(text):
    ...
    return list(word for word, commonality in sorted(ct.items(), key=lambda i: i[1])[::-1])

def translation_table(text):
    detected_language = translate_client.detect_language(text)['language']
    word_set = sort_by_commonality(text)
    translated_words = [unescape(translate_client.translate(word, source_language=detected_language, target_language="en")['translatedText']) for word in word_set]
